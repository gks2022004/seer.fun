...
export async function fetchMarketAccount(
  ...
  try {
    ...
    // Question (4 bytes length prefix + actual string bytes - NOT fixed 200 bytes!)
    // Anchor serializes String as: 4-byte length + actual UTF-8 bytes
    const questionLen = data.readUInt32LE(offset);
    offset += 4;
    const question = data.slice(offset, offset + questionLen).toString("utf-8");
    offset += questionLen; // Skip only the actual string length, NOT 200 bytes
